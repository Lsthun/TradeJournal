...
# Optional: Pillow improves in-app iChart image handling.
try:
...
class TradeJournalModel:
    ...
    def compute_summary(self, account_filter: Optional[str] = None, *, closed_only: bool = False,
        ...
        when ``closed_only`` is True. Only trades with an exit date are considered for statistics. The
        returned dictionary contains keys: total_pnl, num_trades, num_wins, num_losses, win_ratio,
        avg_pnl, avg_hold, avg_winner_pnl_pct, avg_loser_pnl_pct, avg_hold_winners, and avg_hold_losers.
        """
        total_pnl = 0.0
        num_trades = 0
        num_wins = 0
        ...
        for trade in self.trades:
            # Only consider trades with an exit date (closed legs)
...
class TradeJournalApp:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.current_ichart_symbol: Optional[str] = None
        self.chart_canvas = None
        self.ichart_photo = None
        self.ichart_df_cache: Optional[pd.DataFrame] = None
        self.ichart_x_labels: Optional[List[str]] = None
        self.ichart_view_start_idx: Optional[int] = None
        self.ichart_view_end_idx: Optional[int] = None
        self.ichart_drag_start_x: Optional[int] = None
        self.ichart_drag_rect_id: Optional[int] = None
        self.ichart_resize_after_id: Optional[str] = None
        self.ichart_last_hover_idx: Optional[int] = None
    ...
    def _build_ui(self) -> None:
        ...
        # Row 3: Clear filters and toggle table buttons
        clear_filter_btn = ttk.Button(top_frame, text="Clear Filters", command=self.clear_filters)
        clear_filter_btn.grid(row=3, column=0, columnspan=2, padx=(0, 5), pady=2, sticky="w")
        ...
        self.toggle_btn.grid(row=3, column=2, columnspan=2, padx=(0, 5), pady=2, sticky="w")
        ...
        # TAB 3: iChart (Plotly-based)
        ichart_frame = ttk.Frame(self.notebook)
        self.notebook.add(ichart_frame, text="iChart")
        self._build_ichart_tab(ichart_frame)
    ...
    def _build_chart_tab(self, parent_frame: ttk.Frame) -> None:
        ...
        self.chart_trades_tree = ttk.Treeview(trades_frame, columns=("entry", "entry_price", "exit", "exit_price", "qty", "pnl", "pnl_pct"), show="headings", height=6)
        for col, heading in [("entry", "Entry"), ("entry_price", "Entry Price"), ("exit", "Exit"), ("exit_price", "Exit Price"), ("qty", "Qty"), ("pnl", "P&L"), ("pnl_pct", "P&L %")]:
    ...
    def _build_ichart_tab(self, parent_frame: ttk.Frame) -> None:
        """Build the iChart tab (Plotly-based candlestick chart)."""
        top_frame = ttk.Frame(parent_frame)
        top_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)

        self.ichart_controls_visible = True
        self.ichart_collapse_btn = ttk.Button(top_frame, text="−", width=2, command=self._toggle_ichart_controls)
        self.ichart_collapse_btn.pack(side=tk.LEFT, padx=(0, 5))

        ttk.Label(top_frame, text="Symbol:").pack(side=tk.LEFT, padx=(0, 5))
        self.ichart_symbol_var = tk.StringVar()
        self.ichart_symbol_combo = ttk.Combobox(top_frame, textvariable=self.ichart_symbol_var, state="normal", width=15)
        self.ichart_symbol_combo.pack(side=tk.LEFT, padx=(0, 10))
        self.ichart_symbol_combo.bind("<<ComboboxSelected>>", self.on_ichart_symbol_selected)
        self.ichart_symbol_combo.bind("<Return>", self._validate_and_select_ichart_symbol)
        self.ichart_symbol_combo.bind("<KeyRelease>", self._auto_capitalize_ichart_symbol)

        self.ichart_download_btn = ttk.Button(top_frame, text="Download Data", command=self.on_ichart_download_price_data)
        self.ichart_download_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.ichart_open_btn = ttk.Button(top_frame, text="Open Interactive", command=self.open_ichart_in_browser)
        self.ichart_open_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.ichart_status_var = tk.StringVar(value="Ready")
        ttk.Label(top_frame, textvariable=self.ichart_status_var, foreground="cyan").pack(side=tk.LEFT, padx=(0, 5))

        # Hover OHLC readout (for in-app static chart interactivity)
        self.ichart_hover_var = tk.StringVar(value="")
        ttk.Label(top_frame, textvariable=self.ichart_hover_var, foreground="white").pack(side=tk.LEFT, padx=(10, 5))

        self.ichart_controls_frame = ttk.Frame(parent_frame)
        self.ichart_controls_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)

        # Build MA controls once (avoid destroying/recreating widgets)
        self._build_ichart_controls_widgets()

        self.ichart_paned = ttk.PanedWindow(parent_frame, orient=tk.VERTICAL)
        self.ichart_paned.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        self.ichart_display_frame = ttk.Frame(self.ichart_paned)
        self.ichart_paned.add(self.ichart_display_frame, weight=3)

        # Use a Canvas so we can support drag-zoom and better fitting.
        self.ichart_canvas = tk.Canvas(self.ichart_display_frame, highlightthickness=0, background="#1e1e1e")
        self.ichart_canvas.pack(fill=tk.BOTH, expand=True)
        self.ichart_canvas_empty_text_id = self.ichart_canvas.create_text(
            10,
            10,
            anchor="nw",
            fill="#CCCCCC",
            text="Select a symbol to view iChart",
            font=("TkDefaultFont", 12),
        )

        # Interactivity for in-app static rendering: drag-zoom + mousewheel zoom + hover OHLC
        self.ichart_canvas.bind("<ButtonPress-1>", self._ichart_on_drag_start)
        self.ichart_canvas.bind("<B1-Motion>", self._ichart_on_drag_motion)
        self.ichart_canvas.bind("<ButtonRelease-1>", self._ichart_on_drag_end)
        self.ichart_canvas.bind("<Motion>", self._ichart_on_hover)
        # macOS/Windows: MouseWheel. Linux: Button-4/5
        self.ichart_canvas.bind("<MouseWheel>", self._ichart_on_mousewheel)
        self.ichart_canvas.bind("<Button-4>", lambda e: self._ichart_zoom_by_factor(0.85))
        self.ichart_canvas.bind("<Button-5>", lambda e: self._ichart_zoom_by_factor(1.15))

        trades_frame = ttk.Frame(self.ichart_paned)
        self.ichart_paned.add(trades_frame, weight=1)

        ttk.Label(trades_frame, text="Trades for Symbol:").pack(anchor="w", padx=5, pady=2)

        self.ichart_trades_tree = ttk.Treeview(
            trades_frame,
            columns=("entry", "entry_price", "exit", "exit_price", "qty", "pnl", "pnl_pct"),
            show="headings",
            height=6,
        )
        for col, heading in [
            ...
            self.ichart_trades_tree.heading(col, text=heading)
            self.ichart_trades_tree.column(col, width=70, anchor=tk.CENTER if col in ("qty", "pnl_pct") else tk.E)

        vsb = ttk.Scrollbar(trades_frame, orient="vertical", command=self.ichart_trades_tree.yview)
        self.ichart_trades_tree.configure(yscrollcommand=vsb.set)
        self.ichart_trades_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5, pady=2)
        vsb.pack(side=tk.RIGHT, fill="y")
    ...
    def _toggle_ichart_controls(self) -> None:
        """Toggle visibility of iChart control frame."""
        if self.ichart_controls_visible:
            self.ichart_controls_frame.pack_forget()
            self.ichart_collapse_btn.config(text="+")
            self.ichart_controls_visible = False
        else:
            before_widget = getattr(self, "ichart_paned", None)
            if before_widget is not None:
                self.ichart_controls_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5, before=before_widget)
            else:
                self.ichart_controls_frame.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)
            self.ichart_collapse_btn.config(text="−")
            self.ichart_controls_visible = True

    def _auto_capitalize_ichart_symbol(self, event=None) -> None:
        """Auto-capitalize symbol input for iChart as user types."""
        current = self.ichart_symbol_var.get()
        if current != current.upper():
            cursor_pos = self.ichart_symbol_combo.index(tk.INSERT)
            self.ichart_symbol_var.set(current.upper())
            self.ichart_symbol_combo.icursor(cursor_pos)

    def _validate_and_select_ichart_symbol(self, event=None) -> None:
        """Validate typed symbol and match to journal symbols for iChart on Enter key."""
        typed = self.ichart_symbol_var.get().upper()
        ...
        if typed in available_upper:
            ...
            self.ichart_symbol_var.set(available[idx])
            self.on_ichart_symbol_selected()
        else:
            messagebox.showerror(
                "Invalid Symbol",
                f"'{typed}' not found in trade journal.\n\nAvailable symbols: {', '.join(available)}",
            )
            self.ichart_symbol_var.set("")

    def on_ichart_symbol_selected(self, event: Optional[tk.Event] = None) -> None:
        """Handle symbol selection in the iChart tab."""
        symbol = self.ichart_symbol_var.get()
        if symbol:
            self.current_ichart_symbol = symbol
            if self.price_manager.has_data(symbol):
                self.display_ichart(symbol)
            else:
                self.ichart_status_var.set(f"No data for {symbol}. Click 'Download Data' to fetch.")

    def on_ichart_download_price_data(self) -> None:
        """Handle downloading price data for the selected symbol (iChart)."""
        symbol = self.ichart_symbol_var.get()
        if not symbol:
        ...
        if not HAS_YFINANCE:
            messagebox.showerror("Missing Dependency", "yfinance is required. Install with: pip install yfinance")
            return
        ...
        self.ichart_status_var.set(f"Fetching {symbol} ({start_date} to {end_date})...")
        self.root.update()

        try:
            df = self.price_manager.fetch_and_store(symbol, start_date, end_date)
            if df is not None and not df.empty:
                self.ichart_status_var.set(f"✓ Loaded {len(df)} days for {symbol}")
                self.display_ichart(symbol)
            else:
                self.ichart_status_var.set(f"✗ No data found for {symbol}")
                messagebox.showinfo("No Data", f"yfinance returned no data for {symbol}.")
        except Exception as e:
            error_msg = str(e)
            self.ichart_status_var.set(f"✗ Error: {error_msg[:50]}")
            messagebox.showerror("Download Error", f"Failed to download data:\n\n{error_msg}")

    def _populate_ichart_trades_list(self, symbol: str) -> None:
        """Populate the trades list in the iChart tab for the selected symbol."""
        for item in self.ichart_trades_tree.get_children():
            self.ichart_trades_tree.delete(item)
        ...
        for trade in trades_for_symbol:
            ...
            self.ichart_trades_tree.insert("", "end", values=(entry_date_str, entry_price_str, exit_date_str, exit_price_str, qty_str, pnl_str, pnl_pct_str))

    def open_ichart_in_browser(self) -> None:
        """Open the current iChart as an interactive Plotly chart in the browser."""
        symbol = self.ichart_symbol_var.get().strip() or (self.current_ichart_symbol or "")
        if not symbol:
            messagebox.showinfo("No Symbol", "Please select a symbol first.")
            return
        if not HAS_PLOTLY:
            messagebox.showerror("Missing Dependency", "plotly is required. Install with: pip install plotly")
            return
        try:
            # Get MA settings
            try:
                p1 = int(self.ichart_ma1_period_var.get())
                p2 = int(self.ichart_ma2_period_var.get())
                p3 = int(self.ichart_ma3_period_var.get())
            except Exception:
                p1, p2, p3 = 20, 50, 200

            fig = self._build_plotly_figure(
                symbol,
                ma1=(p1, self.ichart_ma1_type_var.get(), self.ichart_ma1_color_var.get(), self.ichart_ma1_enabled_var.get()),
                ma2=(p2, self.ichart_ma2_type_var.get(), self.ichart_ma2_color_var.get(), self.ichart_ma2_enabled_var.get()),
                ma3=(p3, self.ichart_ma3_type_var.get(), self.ichart_ma3_color_var.get(), self.ichart_ma3_enabled_var.get()),
            )
            import tempfile
            ...
            messagebox.showerror("iChart Error", f"Failed to open interactive chart:\n\n{e}")

    def _build_ichart_controls_widgets(self) -> None:
        """Build iChart moving-average controls once and keep references."""
        for widget in self.ichart_controls_frame.winfo_children():
            widget.destroy()

        saved_settings = load_chart_settings()
        ttk.Label(self.ichart_controls_frame, text="Moving Averages:").pack(side=tk.LEFT, padx=5)

        # MA1
        self.ichart_ma1_enabled_var = tk.BooleanVar(value=saved_settings.get("ema1_enabled", True))
        ttk.Checkbutton(self.ichart_controls_frame, text="MA1", variable=self.ichart_ma1_enabled_var).pack(side=tk.LEFT, padx=(10, 3))
        self.ichart_ma1_type_var = tk.StringVar(value=saved_settings.get("ema1_type", "EMA"))
        ttk.Combobox(self.ichart_controls_frame, textvariable=self.ichart_ma1_type_var, values=["EMA", "SMA"], state="readonly", width=4).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Label(self.ichart_controls_frame, text="P:").pack(side=tk.LEFT, padx=(5, 0))
        self.ichart_ma1_period_var = tk.StringVar(value=str(saved_settings.get("ema1_period", 20)))
        ttk.Spinbox(self.ichart_controls_frame, from_=1, to=200, textvariable=self.ichart_ma1_period_var, width=4).pack(side=tk.LEFT, padx=(0, 3))
        self.ichart_ma1_color_var = tk.StringVar(value=saved_settings.get("ema1_color", "blue"))
        ma1_hex = name_to_hex(self.ichart_ma1_color_var.get())
        ma1_swatch = tk.Canvas(self.ichart_controls_frame, width=20, height=12, highlightthickness=1, highlightbackground="#888")
        ma1_rect = ma1_swatch.create_rectangle(0, 0, 20, 12, fill=ma1_hex, outline=ma1_hex)
        ma1_swatch.pack(side=tk.LEFT, padx=(0, 2))
        ttk.Button(
            self.ichart_controls_frame,
            text="C",
            width=1,
            command=lambda: self._pick_color(
                self.ichart_ma1_color_var,
                on_color_change=lambda hx: ma1_swatch.itemconfig(ma1_rect, fill=hx, outline=hx),
            ),
        ).pack(side=tk.LEFT, padx=(0, 8))

        # MA2
        self.ichart_ma2_enabled_var = tk.BooleanVar(value=saved_settings.get("ema2_enabled", True))
        ttk.Checkbutton(self.ichart_controls_frame, text="MA2", variable=self.ichart_ma2_enabled_var).pack(side=tk.LEFT, padx=(10, 3))
        self.ichart_ma2_type_var = tk.StringVar(value=saved_settings.get("ema2_type", "EMA"))
        ttk.Combobox(self.ichart_controls_frame, textvariable=self.ichart_ma2_type_var, values=["EMA", "SMA"], state="readonly", width=4).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Label(self.ichart_controls_frame, text="P:").pack(side=tk.LEFT, padx=(5, 0))
        self.ichart_ma2_period_var = tk.StringVar(value=str(saved_settings.get("ema2_period", 50)))
        ttk.Spinbox(self.ichart_controls_frame, from_=1, to=200, textvariable=self.ichart_ma2_period_var, width=4).pack(side=tk.LEFT, padx=(0, 3))
        self.ichart_ma2_color_var = tk.StringVar(value=saved_settings.get("ema2_color", "orange"))
        ma2_hex = name_to_hex(self.ichart_ma2_color_var.get())
        ma2_swatch = tk.Canvas(self.ichart_controls_frame, width=20, height=12, highlightthickness=1, highlightbackground="#888")
        ma2_rect = ma2_swatch.create_rectangle(0, 0, 20, 12, fill=ma2_hex, outline=ma2_hex)
        ma2_swatch.pack(side=tk.LEFT, padx=(0, 2))
        ttk.Button(
            self.ichart_controls_frame,
            text="C",
            width=1,
            command=lambda: self._pick_color(
                self.ichart_ma2_color_var,
                on_color_change=lambda hx: ma2_swatch.itemconfig(ma2_rect, fill=hx, outline=hx),
            ),
        ).pack(side=tk.LEFT, padx=(0, 8))

        # MA3
        self.ichart_ma3_enabled_var = tk.BooleanVar(value=saved_settings.get("ema3_enabled", False))
        ttk.Checkbutton(self.ichart_controls_frame, text="MA3", variable=self.ichart_ma3_enabled_var).pack(side=tk.LEFT, padx=(10, 3))
        self.ichart_ma3_type_var = tk.StringVar(value=saved_settings.get("ema3_type", "EMA"))
        ttk.Combobox(self.ichart_controls_frame, textvariable=self.ichart_ma3_type_var, values=["EMA", "SMA"], state="readonly", width=4).pack(side=tk.LEFT, padx=(0, 3))
        ttk.Label(self.ichart_controls_frame, text="P:").pack(side=tk.LEFT, padx=(5, 0))
        self.ichart_ma3_period_var = tk.StringVar(value=str(saved_settings.get("ema3_period", 200)))
        ttk.Spinbox(self.ichart_controls_frame, from_=1, to=200, textvariable=self.ichart_ma3_period_var, width=4).pack(side=tk.LEFT, padx=(0, 3))
        self.ichart_ma3_color_var = tk.StringVar(value=saved_settings.get("ema3_color", "purple"))
        ma3_hex = name_to_hex(self.ichart_ma3_color_var.get())
        ma3_swatch = tk.Canvas(self.ichart_controls_frame, width=20, height=12, highlightthickness=1, highlightbackground="#888")
        ma3_rect = ma3_swatch.create_rectangle(0, 0, 20, 12, fill=ma3_hex, outline=ma3_hex)
        ma3_swatch.pack(side=tk.LEFT, padx=(0, 2))
        ttk.Button(
            self.ichart_controls_frame,
            text="C",
            width=1,
            command=lambda: self._pick_color(
                self.ichart_ma3_color_var,
                on_color_change=lambda hx: ma3_swatch.itemconfig(ma3_rect, fill=hx, outline=hx),
            ),
        ).pack(side=tk.LEFT, padx=(0, 8))

        def validate_periods() -> Optional[Tuple[int, int, int]]:
            try:
                p1 = int(self.ichart_ma1_period_var.get())
                p2 = int(self.ichart_ma2_period_var.get())
                p3 = int(self.ichart_ma3_period_var.get())
                if min(p1, p2, p3) < 1 or max(p1, p2, p3) > 200:
                    raise ValueError
                return p1, p2, p3
            except Exception:
                return None
        ...
        def on_update() -> None:
            ...
            if periods is None:
                messagebox.showerror("Invalid Input", "Moving average periods must be numbers between 1 and 200")
                return
            save_chart_settings(
                {
                    "ema1_period": int(self.ichart_ma1_period_var.get()),
                    "ema2_period": int(self.ichart_ma2_period_var.get()),
                    "ema3_period": int(self.ichart_ma3_period_var.get()),
                    "ema1_color": self.ichart_ma1_color_var.get(),
                    "ema2_color": self.ichart_ma2_color_var.get(),
                    "ema3_color": self.ichart_ma3_color_var.get(),
                    "ema1_type": self.ichart_ma1_type_var.get(),
                    "ema2_type": self.ichart_ma2_type_var.get(),
                    "ema3_type": self.ichart_ma3_type_var.get(),
                    "ema1_enabled": self.ichart_ma1_enabled_var.get(),
                    "ema2_enabled": self.ichart_ma2_enabled_var.get(),
                    "ema3_enabled": self.ichart_ma3_enabled_var.get(),
                }
            )
            if self.current_ichart_symbol and self.ichart_df_cache is not None:
                self._render_ichart_image(self.current_ichart_symbol, self.ichart_df_cache)

        ttk.Button(self.ichart_controls_frame, text="Update", command=on_update).pack(side=tk.LEFT, padx=5)
        ttk.Button(self.ichart_controls_frame, text="◀", width=2, command=lambda: self._ichart_pan_by_fraction(-0.2)).pack(side=tk.LEFT, padx=(8, 2))
        ttk.Button(self.ichart_controls_frame, text="▶", width=2, command=lambda: self._ichart_pan_by_fraction(0.2)).pack(side=tk.LEFT, padx=(0, 2))
        ttk.Button(self.ichart_controls_frame, text="Reset", command=self._ichart_reset_view).pack(side=tk.LEFT, padx=(6, 0))

    def display_ichart(self, symbol: str) -> None:
        """Display Plotly chart for the given symbol (static image in-app; interactive fallback)."""
        if not HAS_PLOTLY:
            messagebox.showerror("Missing Dependency", "plotly is required. Install with: pip install plotly")
            return
        ...
        try:
            ...
            if df is None or df.empty:
                self.ichart_status_var.set(f"No price data available for {symbol}")
                return

            self._populate_ichart_trades_list(symbol)

            # Cache data and reset view on first render for a symbol
            self.ichart_df_cache = df
            self.ichart_x_labels = [pd.to_datetime(ts).strftime("%Y-%m-%d") for ts in pd.to_datetime(df.index)]
            if self.ichart_view_start_idx is None or self.ichart_view_end_idx is None or self.current_ichart_symbol != symbol:
                self.ichart_view_start_idx = 0
                self.ichart_view_end_idx = max(0, len(df) - 1)
            self.current_ichart_symbol = symbol

            self._render_ichart_image(symbol, df)
            self.ichart_status_var.set(f"iChart for {symbol} (Plotly)")

        except Exception as e:
            error_msg = str(e)
            self.ichart_status_var.set(f"Error displaying iChart: {error_msg[:50]}")
            messagebox.showerror("iChart Error", f"Failed to display iChart:\n\n{error_msg}")
    ...
    def _build_plotly_figure(
        ...
        *,
        ...
        """Create a Plotly candlestick figure with trade markers and optional moving averages."""
        if not HAS_PLOTLY:
        ...
        # Use a category x-axis (string dates) so zoomed views don't show gaps for missing market days.
        x_dt = pd.to_datetime(df.index)
        x = [d.strftime("%Y-%m-%d") for d in x_dt]
        fig = go.Figure(
            data=[
                go.Candlestick(
                    x=x,
        ...
        def calc_ma(close_series: pd.Series, period: int, ma_type: str) -> pd.Series:
            if ma_type == "SMA":
        ...
        for ma in (ma1, ma2, ma3):
            if not ma:
                continue
            period, ma_type, color_value, enabled = ma
            if not enabled:
                continue
            y = calc_ma(df['close'], int(period), str(ma_type))
            fig.add_trace(
                go.Scatter(
                    x=x,
                    y=y,
                    mode='lines',
                    line=dict(color=name_to_hex(str(color_value)), width=2),
                    name=f"{ma_type} {period}",
                )
            )

        # Trade markers + stacked qty labels per day
        trades_for_symbol = [t for t in self.model.trades if t.symbol.upper() == symbol.upper()]
        buy_points: List[Tuple[pd.Timestamp, float, float]] = []
        sell_points: List[Tuple[pd.Timestamp, float, float]] = []
        for t in trades_for_symbol:
            if t.entry_date and t.entry_price is not None:
                buy_points.append((pd.to_datetime(t.entry_date), float(t.entry_price), float(t.quantity)))
            if t.exit_date and t.exit_price is not None:
                sell_points.append((pd.to_datetime(t.exit_date), float(t.exit_price), float(t.quantity)))

        if buy_points:
            fig.add_trace(
                go.Scatter(
                    x=[x[min(max(int(x_dt.get_indexer([p[0]], method='nearest')[0]), 0), len(x) - 1)] for p in buy_points],
        ...
        if sell_points:
            fig.add_trace(
                go.Scatter(
                    x=[x[min(max(int(x_dt.get_indexer([p[0]], method='nearest')[0]), 0), len(x) - 1)] for p in sell_points],
                    y=[p[1] for p in sell_points],
                    mode='markers',
        ...
        def add_qty_annotations(points: List[Tuple[pd.Timestamp, float, float]], color: str, above: bool) -> None:
            from collections import defaultdict
            grouped: Dict[pd.Timestamp, List[Tuple[float, float]]] = defaultdict(list)
            for ts, y_val, qty in points:
                idx = int(x_dt.get_indexer([pd.to_datetime(ts)], method='nearest')[0])
                idx = min(max(idx, 0), len(x) - 1)
                grouped[pd.Timestamp(x[idx])].append((y_val, qty))
            for day, items in grouped.items():
                for i, (y_val, qty) in enumerate(items):
                    fig.add_annotation(
                        x=str(day)[:10],
                        y=y_val,
                        text=str(int(qty)) if abs(qty - int(qty)) < 1e-6 else f"{qty:.2f}",
                        showarrow=False,
                        font=dict(color=color, size=10),
                        yshift=(-18 * i) if above else (18 * i),
                    )

        add_qty_annotations(buy_points, color='green', above=True)
        add_qty_annotations(sell_points, color='red', above=False)

        # Axis + layout: hide rangeslider (the "markup" under the chart), and set category axis.
        fig.update_xaxes(type="category", rangeslider_visible=False, tickmode="linear", tick0=0, dtick=max(1, len(x) // 15))

        if view_range is not None:
            start_idx, end_idx = view_range
            start_idx = max(0, min(int(start_idx), len(x) - 1))
            end_idx = max(0, min(int(end_idx), len(x) - 1))
            if end_idx < start_idx:
                start_idx, end_idx = end_idx, start_idx
            # For category axes, Plotly accepts numeric ranges in category index space.
            fig.update_xaxes(range=[start_idx, end_idx])

        fig.update_layout(
            title=f"{symbol} Price Chart (iChart)",
            xaxis_title="Date",
            yaxis_title="Price",
    ...
    def _ichart_get_render_size(self) -> Tuple[int, int]:
        """Return the desired PNG render size based on current canvas size."""
        canvas = getattr(self, "ichart_canvas", None)
        if canvas is None:
            return 1200, 500
        w = int(canvas.winfo_width() or 0)
        h = int(canvas.winfo_height() or 0)
        # During initial layout, Tk can report 1px sizes. Default to something reasonable.
        if w < 100:
            w = 1200
        if h < 100:
            h = 500
    ...
    def _render_ichart_image(self, symbol: str, df: pd.DataFrame) -> None:
        """Render the iChart image into the canvas using Plotly + kaleido (non-blocking)."""
        if not HAS_PLOTLY:
            return
        canvas = getattr(self, "ichart_canvas", None)
        if canvas is None:
        ...
        # Pull settings from the control vars
        try:
            p1 = int(self.ichart_ma1_period_var.get())
            p2 = int(self.ichart_ma2_period_var.get())
            p3 = int(self.ichart_ma3_period_var.get())
        ...
        w, h = self._ichart_get_render_size()
        start_idx = self.ichart_view_start_idx or 0
        end_idx = self.ichart_view_end_idx if self.ichart_view_end_idx is not None else max(0, len(df) - 1)
        
        # Use a lower scale for faster rendering; Plotly will still look good
        fig = self._build_plotly_figure(
            symbol,
            df=df,
            ma1=(p1, self.ichart_ma1_type_var.get(), self.ichart_ma1_color_var.get(), self.ichart_ma1_enabled_var.get()),
            ...
            width=w,
            height=h,
            view_range=(start_idx, end_idx),
        )

        try:
            # Use scale=1.0 for much faster rendering
            png_bytes = fig.to_image(format="png", width=w, height=h, scale=1.0)
        except Exception:
            self.ichart_status_var.set("Install kaleido for in-app rendering; use Open Interactive")
            return

        import io
        canvas.delete("ichart_img")
        canvas.itemconfigure(getattr(self, "ichart_canvas_empty_text_id", 0), state="hidden")
        ...
        try:
            if HAS_PIL:
                ...
                # Tk PhotoImage can't load bytes directly; use tempfile fallback.
                import tempfile
            ...
            self.ichart_photo = photo
            canvas.create_image(0, 0, anchor="nw", image=photo, tags=("ichart_img",))
        except Exception as e:
            self.ichart_status_var.set(f"Render error: {str(e)[:40]}")

    def _ichart_on_resize(self, event=None) -> None:
        pass  # Removed: don't re-render on every resize; just render once on symbol selection

    def _ichart_on_drag_start(self, event) -> None:
        if self.ichart_df_cache is None or self.ichart_x_labels is None:
            return
        self.ichart_drag_start_x = int(event.x)
        if self.ichart_drag_rect_id is not None:
            try:
                self.ichart_canvas.delete(self.ichart_drag_rect_id)
            except Exception:
                pass
        self.ichart_drag_rect_id = self.ichart_canvas.create_rectangle(
            event.x,
            0,
            event.x,
            self.ichart_canvas.winfo_height(),
            outline="#AAAAAA",
            width=2,
            fill="",
        )

    def _ichart_on_drag_motion(self, event) -> None:
        if self.ichart_drag_start_x is None or self.ichart_drag_rect_id is None:
            return
        # Just show the selection rectangle; don't re-render (expensive).
        try:
            self.ichart_canvas.coords(self.ichart_drag_rect_id, self.ichart_drag_start_x, 0, int(event.x), self.ichart_canvas.winfo_height())
        except Exception:
            pass

    def _ichart_on_drag_end(self, event) -> None:
        if self.ichart_df_cache is None or self.ichart_x_labels is None:
            return
        if self.ichart_drag_start_x is None:
            return
        w = max(1, int(self.ichart_canvas.winfo_width()))
        x0 = min(self.ichart_drag_start_x, int(event.x))
        x1 = max(self.ichart_drag_start_x, int(event.x))
        if self.ichart_drag_rect_id is not None:
            try:
                self.ichart_canvas.delete(self.ichart_drag_rect_id)
            except Exception:
                pass
            self.ichart_drag_rect_id = None
        self.ichart_drag_start_x = None

        # Ignore tiny drags
        if x1 - x0 < 10:
            return

        view_start = self.ichart_view_start_idx or 0
        view_end = self.ichart_view_end_idx if self.ichart_view_end_idx is not None else max(0, len(self.ichart_x_labels) - 1)
        span = max(1, view_end - view_start)

        # Map pixel positions to data indices (relative to current view)
        rel0 = x0 / w
        rel1 = x1 / w
        new_start = view_start + int(rel0 * span)
        new_end = view_start + int(rel1 * span)
        new_start = max(0, min(new_start, len(self.ichart_x_labels) - 1))
        new_end = max(0, min(new_end, len(self.ichart_x_labels) - 1))
        
        # Only zoom if selection is meaningful
        if abs(new_end - new_start) < 5:
            return
        
        self.ichart_view_start_idx, self.ichart_view_end_idx = (min(new_start, new_end), max(new_start, new_end))
        if self.current_ichart_symbol:
            self._render_ichart_image(self.current_ichart_symbol, self.ichart_df_cache)

    def _ichart_on_mousewheel(self, event) -> None:
        # On macOS, delta is typically small +/- values; on Windows larger steps.
        if event.delta == 0:
            return
        factor = 0.9 if event.delta > 0 else 1.1
        self._ichart_zoom_by_factor(factor)

    def _ichart_zoom_by_factor(self, factor: float) -> None:
        if self.ichart_x_labels is None or self.ichart_df_cache is None:
            return
        start = self.ichart_view_start_idx or 0
        end = self.ichart_view_end_idx if self.ichart_view_end_idx is not None else max(0, len(self.ichart_x_labels) - 1)
        if end <= start:
            return
        ...
        # Clamp zoom
        new_half = max(5.0, min(new_half, (len(self.ichart_x_labels) - 1) / 2.0))
        new_start = int(center - new_half)
        new_end = int(center + new_half)
        new_start = max(0, new_start)
        new_end = min(len(self.ichart_x_labels) - 1, new_end)
        if new_end - new_start < 5:
            return
        self.ichart_view_start_idx, self.ichart_view_end_idx = new_start, new_end
        if self.current_ichart_symbol:
            self._render_ichart_image(self.current_ichart_symbol, self.ichart_df_cache)

    def _ichart_pan_by_fraction(self, frac: float) -> None:
        if self.ichart_x_labels is None or self.ichart_df_cache is None:
            return
        start = self.ichart_view_start_idx or 0
        end = self.ichart_view_end_idx if self.ichart_view_end_idx is not None else max(0, len(self.ichart_x_labels) - 1)
        ...
        if new_start < 0:
            new_end -= new_start
            new_start = 0
        if new_end > len(self.ichart_x_labels) - 1:
            over = new_end - (len(self.ichart_x_labels) - 1)
            new_start -= over
            new_end = len(self.ichart_x_labels) - 1
            new_start = max(0, new_start)
        self.ichart_view_start_idx, self.ichart_view_end_idx = new_start, new_end
        if self.current_ichart_symbol:
            self._render_ichart_image(self.current_ichart_symbol, self.ichart_df_cache)

    def _ichart_reset_view(self) -> None:
        if self.ichart_x_labels is None or self.ichart_df_cache is None:
            return
        self.ichart_view_start_idx = 0
        self.ichart_view_end_idx = len(self.ichart_x_labels) - 1
        if self.current_ichart_symbol:
            self._render_ichart_image(self.current_ichart_symbol, self.ichart_df_cache)

    def _ichart_on_hover(self, event) -> None:
        """Show OHLC data at cursor position without re-rendering the image."""
        if self.ichart_df_cache is None or self.ichart_x_labels is None:
            return
        w = max(1, int(self.ichart_canvas.winfo_width()))
        x = max(0, min(int(event.x), w - 1))
        view_start = self.ichart_view_start_idx or 0
        view_end = self.ichart_view_end_idx if self.ichart_view_end_idx is not None else max(0, len(self.ichart_x_labels) - 1)
        span = max(1, view_end - view_start)
        idx = view_start + int((x / w) * span)
        idx = max(0, min(idx, len(self.ichart_df_cache) - 1))
        if self.ichart_last_hover_idx == idx:
            return
        self.ichart_last_hover_idx = idx
        row = self.ichart_df_cache.iloc[idx]
        date_str = self.ichart_x_labels[idx] if idx < len(self.ichart_x_labels) else ""
        try:
            self.ichart_hover_var.set(
                f"{date_str}  O:{float(row['open']):.2f}  H:{float(row['high']):.2f}  L:{float(row['low']):.2f}  C:{float(row['close']):.2f}"
            )
        except Exception:
            self.ichart_hover_var.set("")
    ...
    def _populate_chart_trades_list(self, symbol: str) -> None:
        ...
        for trade in trades_for_symbol:
            ...
            self.chart_trades_tree.insert("", "end", values=(entry_date_str, entry_price_str, exit_date_str, exit_price_str, qty_str, pnl_str, pnl_pct_str))
    ...
    def _plot_candlestick_with_indicators(self, symbol: str, df: pd.DataFrame, trades_for_symbol: list,
        ...
        # Create toolbar buttons with custom command handlers
        def toolbar_home():
            self.chart_canvas.figure.canvas.toolbar.home()
        def toolbar_back():
            self.chart_canvas.figure.canvas.toolbar.back()
        def toolbar_forward():
            self.chart_canvas.figure.canvas.toolbar.forward()
        def toolbar_pan():
            self.chart_canvas.figure.canvas.toolbar.pan()
        def toolbar_zoom():
            self.chart_canvas.figure.canvas.toolbar.zoom()
        def toolbar_save():
            self.chart_canvas.figure.canvas.toolbar.save_figure()

        ttk.Button(text_toolbar, text="Home", command=toolbar_home).pack(side=tk.LEFT, padx=2)
        ttk.Button(text_toolbar, text="Back", command=toolbar_back).pack(side=tk.LEFT, padx=2)
        ttk.Button(text_toolbar, text="Forward", command=toolbar_forward).pack(side=tk.LEFT, padx=2)
        ttk.Button(text_toolbar, text="Pan", command=toolbar_pan).pack(side=tk.LEFT, padx=2)
        ttk.Button(text_toolbar, text="Zoom", command=toolbar_zoom).pack(side=tk.LEFT, padx=2)
        ttk.Button(text_toolbar, text="Save", command=toolbar_save).pack(side=tk.LEFT, padx=2)
    ...
    def update_ichart_symbols(self) -> None:
        """Update the list of available symbols in the iChart tab."""
        symbols = sorted(set(t.symbol for t in self.model.trades))
        self.ichart_symbol_combo['values'] = symbols
        if symbols and not self.ichart_symbol_var.get():
            self.ichart_symbol_var.set(symbols[0])
    ...
    def update_summary_and_chart(self) -> None:
        ...
        # Determine summary based on top filter
        if top_set is None:
        ...
        if summary["num_trades"] == 0:
            ...
            summary_text = (
                ...
                f"Average P&L: {summary['avg_pnl']:.2f}\n"
                f"Average Hold (days): {summary['avg_hold']:.1f}\n"
